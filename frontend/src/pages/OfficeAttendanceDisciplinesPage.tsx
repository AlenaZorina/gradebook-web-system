...
import type { LoginResponse, OfficeAttendanceDiscipline } from "../api";
import { getOfficeAttendanceDisciplines } from "../api";
import "./TeacherSchedulePage.css";
import "./OfficeAttendanceDisciplinesPage.css";

type OfficeAttendanceDisciplinesPageProps = {
  user: LoginResponse;
  onLogout: () => void;
...
export function OfficeAttendanceDisciplinesPage({
  ...
  onOpenStudents,
  onOpenAnalytics
}: OfficeAttendanceDisciplinesPageProps) {
  const [disciplines, setDisciplines] = useState<OfficeAttendanceDiscipline[]>([]);
  ...
  useEffect(() => {
    async function loadDisciplines() {
      try {
        ...
        const data = await getOfficeAttendanceDisciplines(user.idUser);
        setDisciplines(data);
      } catch (err) {
        setError(
          err instanceof Error
            ? err.message
            : "Ошибка загрузки дисциплин для посещаемости"
        );
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <div className="schedule-layout">
      ...
      <main className="office-attendance-disciplines-content">
        ...
        <section className="office-attendance-disciplines-filters">
          <select
        ...
        {!isLoading && !error && filteredDisciplines.length > 0 && (
          <section className="office-attendance-disciplines-grid">
            {filteredDisciplines.map((discipline) => (
              <article
                className="office-attendance-discipline-card"
              ...
              >
                ...
                <div className="office-attendance-discipline-body">
                  ...
                  <div className="office-attendance-discipline-tags">
