...
import type {
  ...
  OfficeStudentGradebookDiscipline
} from "../api";
import {
  getOfficeStudentDetails,
  getOfficeStudentGradebookSummary
} from "../api";
import "./TeacherSchedulePage.css";
import "./OfficeStudentGradebookSummaryPage.css";

type OfficeStudentGradebookSummaryPageProps = {
  user: LoginResponse;
  studentId: number;
  onLogout: () => void;
  onBack: () => void;
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const [studentData, gradebookData] = await Promise.all([
          getOfficeStudentDetails(user.idUser, studentId),
          getOfficeStudentGradebookSummary(user.idUser, studentId)
        ]);
        ...
        setError(
          err instanceof Error ? err.message : "Ошибка загрузки ведомости студента"
        );
      } finally {
        setIsLoading(false);
      }
