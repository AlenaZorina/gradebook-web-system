...
import type { LoginResponse, OfficeResitDiscipline } from "../api";
import { getOfficeResitDisciplines } from "../api";
import "./TeacherSchedulePage.css";
import "./OfficeResitsPage.css";

type OfficeResitsPageProps = {
  user: LoginResponse;
  onLogout: () => void;
...
export function OfficeResitsPage({
  ...
  onOpenStudents,
  onOpenAnalytics
}: OfficeResitsPageProps) {
  const [disciplines, setDisciplines] = useState<OfficeResitDiscipline[]>([]);
  ...
  useEffect(() => {
    async function loadDisciplines() {
      try {
        ...
        const data = await getOfficeResitDisciplines(user.idUser);
        setDisciplines(data);
      } catch (err) {
        setError(
          err instanceof Error
            ? err.message
            : "Ошибка загрузки дисциплин для пересдач"
        );
      } finally {
        setIsLoading(false);
      }
  ...
  return (
    <div className="schedule-layout">
      ...
      <main className="office-resits-content">
        ...
        <section className="office-resits-filters">
          <select
        ...
        {!isLoading && !error && filteredDisciplines.length > 0 && (
          <section className="office-resits-grid">
            {filteredDisciplines.map((discipline) => (
              <article
                className="office-resit-card"
              ...
              >
                ...
                <div className="office-resit-card-body">
                  ...
                  <div className="office-resit-card-tags">
