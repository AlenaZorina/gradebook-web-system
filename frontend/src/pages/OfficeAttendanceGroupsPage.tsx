...
import type { LoginResponse, OfficeAttendanceGroup } from "../api";
import { getOfficeAttendanceGroups } from "../api";
import "./TeacherSchedulePage.css";
import "./OfficeAttendanceGroupsPage.css";

type OfficeAttendanceGroupsPageProps = {
  user: LoginResponse;
  disciplineId: number;
  onLogout: () => void;
  onBack: () => void;
...
export function OfficeAttendanceGroupsPage({
  ...
  onOpenStudents,
  onOpenAnalytics
}: OfficeAttendanceGroupsPageProps) {
  const [groups, setGroups] = useState<OfficeAttendanceGroup[]>([]);
  ...
  useEffect(() => {
    async function loadGroups() {
      try {
        ...
        const data = await getOfficeAttendanceGroups(user.idUser, disciplineId);
        setGroups(data);
      } catch (err) {
  ...
  return (
    <div className="schedule-layout">
      ...
      <main className="office-attendance-groups-content">
        ...
        <section className="office-attendance-groups-hero">
          <h1>Посещаемость</h1>
