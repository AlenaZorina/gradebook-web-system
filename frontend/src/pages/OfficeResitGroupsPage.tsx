...
import type { LoginResponse, OfficeResitGroup } from "../api";
import { getOfficeResitGroups } from "../api";
import "./TeacherSchedulePage.css";
import "./OfficeResitGroupsPage.css";

type OfficeResitGroupsPageProps = {
  user: LoginResponse;
  disciplineId: number;
  onLogout: () => void;
  onBack: () => void;
...
export function OfficeResitGroupsPage({
  user,
  disciplineId,
  onLogout,
  onBack,
  ...
  onOpenStudents,
  onOpenAnalytics
}: OfficeResitGroupsPageProps) {
  const [groups, setGroups] = useState<OfficeResitGroup[]>([]);
  ...
  useEffect(() => {
    async function loadGroups() {
      try {
        ...
        const data = await getOfficeResitGroups(user.idUser, disciplineId);
        setGroups(data);
      } catch (err) {
  ...
  return (
    <div className="schedule-layout">
      ...
      <main className="office-resit-groups-content">
        ...
        <section className="office-resit-groups-hero">
